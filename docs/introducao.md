...
--8<-- "tests/doctest/test_doc_imc.py:imc_calculo"
```
...
```py title="Cálculo de IMC"
--8<-- "tests/doctest/test_doc_imc.py:imc_valores"
```
...
```py title="Cálculo de IMC"
--8<-- "tests/doctest/test_doc_medida_lista.py:medida_lista"
```
...
```py title="Cálculo de IMC"
--8<-- "tests/doctest/test_doc_medida_lista.py:medida_lista_incerteza"
```
...
--8<-- "tests/doctest/test_doc_equivalencia.py:comparacao"
```
...
```py
--8<-- "tests/doctest/test_doc_sorted_list.py:ordenacao"
```
...
--8<-- "tests/doctest/test_doc_intervalo_de_confianca.py:intervalo_confianca"
```
...
```py
--8<-- "tests/doctest/test_doc_dimensao.py:dimensao"
```

[^1]: Perceba que o LabIFSC2 tem uma diferença de ordem de argumentos na criação de uma `Medida` em relação ao LabIFSC. Eu decidi colocar as unidades no meio da declaração pois, pessoalmente, acho que uma unidade no meio dos argumentos torna a leitura dos valores nominais e incerteza mais simples. Isso fica mais evidente em funções como `lab.linspaceM` e `lab.arrayM` em que temos muitos valores numéricos nos seus construtores.

[^2]: A convenção de desvio padrão é a mesma da apostila, ou seja, dividimos os desvios quadráticos por \(N-1\) e **NÃO** por \(N\). Isso é chamado de correção de [Bessel](https://en.wikipedia.org/wiki/Bessel%27s_correction), esse detalhe só tem efeito visível para valores pequenos de \(N\).
