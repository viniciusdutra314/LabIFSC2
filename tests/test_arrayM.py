...
from LabIFSC2 import (Medida, arrayM, converter_array, converter_array_si,
...
def test_array_m():
    ...
    for i in range(len(x_dados)):
        assert x_dados[i].nominal==i+1
        assert x_dados[i].incerteza==0.01
        assert x_dados[i].unidade=='second'
    with pytest.raises(TypeError):
