...
@pytest.mark.parametrize(
    "a, k", [(3.6, 1.05), (2.0, 0.5), (1.0, -2.0), (4.5, -0.75), (3.0, 1.5), (1, 0)]
)
def test_equivalencia_com_scipy(a, k):
    ...
    assert np.isclose(
        a_scipy.nominal(""), exponencial_np.amplitude.nominal(""), atol=(5e-1) * a
    )
    assert np.isclose(
        k_scipy.nominal(""), exponencial_np.expoente.nominal(""), atol=(1e-2)
    )
    assert np.isclose(a, exponencial_np.amplitude.nominal(""), rtol=1e-2) or np.isclose(
        a, exponencial_np.amplitude.nominal(""), atol=1e-2
    )
    assert np.isclose(k, exponencial_np.expoente.nominal(""), rtol=1e-2) or np.isclose(
        k, exponencial_np.expoente.nominal(""), atol=1e-2
    )
