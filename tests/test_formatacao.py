...
def test_padraolatex():
    ...
    assert f'{x:latex}'== r"(1.50 \pm 0.01) \times 10^{1} \, \text{m}"

def test_mudandobase():
    x=lab.Medida(456,0.3,'nm')
    assert f'{x:latex}' == r"(4.560 \pm 0.003) \times 10^{2} \, \text{nm}"
    assert f'{x}' == "(4.560 ± 0.003)E2 nm"

    assert f'{x:latex_E0}' == r'(456.0 \pm 0.3)\, \text{nm}'
    assert f'{x:E0}' == r"(456.0 ± 0.3) nm"

def test_removeroarredondamento():
    x=lab.Medida(21.53,1,'cm')
    assert f'{x:latex}' == r"(2.2 \pm 0.1) \times 10^{1} \, \text{cm}"
    assert str(x) == r"(2.2 ± 0.1)E1 cm"

    assert f'{x:latex_full}' == r"(2.153 \pm 0.1) \times 10^{1} \, \text{cm}"
...
def teste_negativo():
    ...
    assert f'{x:latex_E-11}'==r'(100.0 \pm 1.0) \times 10^{-11} \, \text{m}'