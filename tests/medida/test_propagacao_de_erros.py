...
def comparar_relativo_ufloat_lab1_lab2(
    ufloat: ufloat, medida_lab1: lab2.Medida, medida_lab2: lab1.Medida, precisao=1e-3
):
...
def comparar_absoluto_ufloat_lab1_lab2(
    ufloat: ufloat, medida_lab1: lab2.Medida, medida_lab2: lab1.Medida, precisao=1e-3
):
...
def test_arithmetic_operations():
    ...
    for value1 in np.linspace(10, 2.6, 10):
        for value2 in np.linspace(25, 3, 10):
            ...
            x_lab1 = lab1.Medida((value1, incerteza), "") + lab1.Medida(
                (value2, incerteza), ""
            )
            x_lab2 = lab2.Medida(
                value1,
            ...
            x_lab2 = lab2.Medida(value1, "", incerteza) - lab2.Medida(
                value2, "", incerteza
            )
            comparar_absoluto_ufloat_lab1_lab2(x, x_lab1, x_lab2)
...
def test_hiperbolicas():
    ...
    for value in np.linspace(0.5, 1, 100):
        ...
        x_lab2 = np.arcsinh(lab2.Medida(value, "", incerteza))
        comparar_ufloat_lab2(x, x_lab2)

        x = umath.acosh(
            ufloat(value + 1, incerteza)
        )  # value + 1 to ensure it's in the domain of acosh
        x_lab2 = np.arccosh(lab2.Medida(value + 1, "", incerteza))
        comparar_ufloat_lab2(x, x_lab2)

        x = umath.atanh(
            ufloat(value / 2, incerteza)
        )  # value / 2 to ensure it's in the domain of atanh
        x_lab2 = np.arctanh(lab2.Medida(value / 2, "", incerteza))
        comparar_ufloat_lab2(x, x_lab2)
...
def test_power():
    ...
    for value in np.linspace(1, 3, 100):
        ...
        x_lab2 = np.power(lab2.Medida(value, "", incerteza), 2)
        comparar_relativo_ufloat_lab1_lab2(x, x_lab1, x_lab2)

        x = umath.pow(ufloat(value, incerteza), 3)
        x_lab1 = lab1.Medida((value, incerteza), "") ** 3
        x_lab2 = np.power(lab2.Medida(value, "", incerteza), 3)
        comparar_relativo_ufloat_lab1_lab2(x, x_lab1, x_lab2)
