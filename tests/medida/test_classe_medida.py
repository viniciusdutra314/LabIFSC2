...
def test_alterar_monte_carlo_rng() -> None:
    lab.alterar_monte_carlo_samples(10)
    try:
        ...
        assert np.all(primeiro_histograma == segundo_histograma)
    finally:
        lab.alterar_monte_carlo_samples(100_000)
        lab.alterar_monte_carlo_rng(np.random.default_rng())
...
def test_inicializacao():
    ...
    try:
        ...
        raise AssertionError("Nao deveria levantar um erro") from erro
