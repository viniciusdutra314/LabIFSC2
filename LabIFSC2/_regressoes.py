...
class AjustePolinomial:
    ...
    def __call__(self, medidas: Medida | Iterable[Medida]) -> Medida | NDArray[np.object_]:
        ...
        if isinstance(medidas, Medida):
            ...
            return np.array(
                [sum([coef * (medida ** i) for i, coef in enumerate(self.coef)], start=zero) for medida in medidas],
                dtype=object)



def _validate_medidas_and_convert_to_list(x: Iterable[Medida], y: Iterable[Medida], *, positivos_x: bool = False, positivos_y: bool = False) -> tuple[list[Medida], list[Medida]]:
    x_list = list(x)
    y_list = list(y)
...
def regressao_polinomial(x_medidas: Iterable[Medida], 
...
def regressao_exponencial(x_medidas: Iterable[Medida],
    ...
    Args:
        ...
        y_medidas (Iterable[Medida]): Coleção de medidas da variável dependente (todos positivos).
    ...
    Raises:
        ...
        ValueError: Se x_medidas e y_medidas não tiverem o mesmo tamanho.
    """
    x_medidas, y_medidas = _validate_medidas_and_convert_to_list(x_medidas, y_medidas, positivos_y=True)
    if base < 1:
...
def regressao_potencia(x_medidas: Iterable[Medida],
                       y_medidas: Iterable[Medida],
    ...
    Args:
        x_medidas (Iterable[Medida]): Coleção de medidas da variável independente (todos positivos).
        y_medidas (Iterable[Medida]): Coleção de medidas da variável dependente (todos positivos).
    ...
    Raises:
        ...
        ValueError: Se x_medidas e y_medidas não tiverem o mesmo tamanho.
    """
    x_medidas, y_medidas = _validate_medidas_and_convert_to_list(x_medidas, y_medidas, positivos_x=True, positivos_y=True)
    unidade_x_si = _obter_unidade_si(x_medidas[0])
    unidade_y_si = _obter_unidade_si(y_medidas[0])
    if x0 is None:
